...
REQUIRED_COLUMNS = ["CUI", "LAT", "SAB", "CODE", "STR"]
...
class UMLSLoader:
    ...
    def preprocess_rrf_to_parquet(self, output_path: Path, progress_callback: Optional[callable] = None) -> Path:
        """
    ...
    def search(
        ...
        if len(exact_matches) > 0:
            ...
            unique_terms = df.select("STR").unique().to_series().to_list()
        ...
        for cui in cuis:
            ...
            all_terms = cui_df.select("STR").unique().to_series().to_list()
            ...
            # Find preferred name (shortest non-acronym term)
            preferred = min(all_terms, key=lambda x: (len(x) < 3, len(x)))
    ...
    def get_synonyms_by_cui(self, cui: str) -> List[str]:
        ...
        return matches.select("STR").unique().to_series().to_list()
